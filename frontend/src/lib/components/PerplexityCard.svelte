<script>
  ...
  $: rawSynth = activeSynth || fetchedSynth || cluster?.precomputed_synthesis;
  $: displaySynth = (rawSynth && !isLowQualityOrEnglish(rawSynth, cluster)) ? rawSynth : null;
  ...
  >
    ...
    <div class="p-6 sm:p-8 space-y-8 flex-1">
      <div class="bg-card border border-border rounded-3xl p-6 space-y-5 shadow-xl">
        ...
        {#if isSynthLoading}
          <div class="space-y-4">
            <div class="flex items-center gap-3 text-xs text-primary bg-primary/10 border border-primary/20 p-3 rounded-xl animate-pulse font-semibold">
